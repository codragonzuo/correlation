...
int producer_test() {
	...
	std::string brokers = "192.168.20.45:6667";
	std::string topic = "snmp";
	...
	for (std::string line; 5 /*run*/ && std::getline(std::cin, line);) {
		if (line.empty()) {
			producer->poll(0);
			continue;
		}
...
int Testmain() {
	...
	topics.push_back("snmp");

	brokers = "192.168.20.45:6667";
